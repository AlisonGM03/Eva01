...
class ToolManager:
    ...
    async def execute(self, client, actions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """ Execute a list of actions """
        
        async def execute_tool(action: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if hasattr(tool, 'run_client') and not result.get("error"): # run the client function if available and no error
                if asyncio.iscoroutinefunction(tool.run_client):
                    client_results = await tool.run_client(client, **result)
                else:
                    client_results = await asyncio.to_thread(tool.run_client, client, **result)
                if client_results:
                    result["additional"] = client_results
                        
            return {"result": result}
        
        tasks = [execute_tool(action) for action in actions]
